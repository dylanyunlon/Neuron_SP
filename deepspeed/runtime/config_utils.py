...
class DeslocConfigCrossChecker:
    """Cross-validate DES-LOC config against DeepSpeed config.
    Checks: ZeRO stage compat, pipeline parallel compat, AMP compat."""

    def __init__(self, ds_config, desloc_config):
        self._ds = ds_config or {}
        self._dl = desloc_config or {}
        self._issues = []

    def check_zero_compat(self):
        stage = self._ds.get("zero_optimization", {}).get("stage", 0)
        Kx = self._dl.get("desloc_Kx", 1)
        if stage >= 3 and Kx > 1:
            self._issues.append("ZeRO-3 + DES-LOC Kx>1: param partitioning may conflict with Kx gating")
        if stage >= 2:
            overlap = self._ds.get("zero_optimization", {}).get("overlap_comm", False)
            if overlap and Kx > 1:
                self._issues.append("overlap_comm=True + Kx>1: async AllReduce timing may be unpredictable")
        return len([i for i in self._issues if "ZeRO" in i]) == 0

    def check_pipeline_compat(self):
        pp = self._ds.get("pipeline", {}).get("enabled", False)
        Kx = self._dl.get("desloc_Kx", 1)
        if pp and Kx > 1:
            self._issues.append("Pipeline parallel + DES-LOC: ensure Kx divides micro_batch count")
        return True

    def check_amp_compat(self):
        amp = self._ds.get("amp", {}).get("enabled", False)
        if amp:
            self._issues.append("Apex AMP + DES-LOC: prefer native BF16 instead")
        return True

    def check_all(self):
        self._issues.clear()
        self.check_zero_compat()
        self.check_pipeline_compat()
        self.check_amp_compat()
        return {"issues": list(self._issues), "n_issues": len(self._issues)}

class DeslocConfigMigrator:
    """Migrate DES-LOC config between versions."""

    V1_KEYS = {"desloc_Kx", "desloc_Ku", "desloc_Kv"}
    V2_KEYS = {"desloc_Kx", "desloc_Ku", "desloc_Kv", "desloc_inner_optimizer", "desloc_outer_optimizer"}

    @staticmethod
    def detect_version(config):
        if "desloc_inner_optimizer" in config:
            return 2
        if "desloc_Kx" in config:
            return 1
        return 0
    ...
    def migrate_v1_to_v2(config):
        new = dict(config)
        new.setdefault("desloc_inner_optimizer", "adam")
        new.setdefault("desloc_outer_optimizer", "average")
        new.setdefault("desloc_warmup_steps", 512)
        new.setdefault("desloc_clip_rho", 1.0)
        return new

    @classmethod
    def ensure_latest(cls, config):
        v = cls.detect_version(config)
        if v == 0:
            return config
        if v == 1:
            return cls.migrate_v1_to_v2(config)
        return config
