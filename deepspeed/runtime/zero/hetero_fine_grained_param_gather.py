...
def register(engine) -> None:
    """Register HeteroGatherConfig on a DeepSpeed engine.

    Instantiates a :class:`HeteroGatherConfig` from the engine's configuration
    and attaches it as ``engine.hetero_fine_grained_param_gather``.
    ...
    """
    logger.info(
        "hetero_fine_grained_param_gather.register() called on engine type=%s",
        type(engine).__name__,
    )

    engine.hetero_fine_grained_param_gather = None
    logger.info("hetero_fine_grained_param_gather.register() attached engine.hetero_fine_grained_param_gather")
