...
# =================================================================
# M085: WSD Schedule + Nesterov Outer Optimizer + Checkpoint Init
# Claude-5 (M077-M091)
# DES-LOC Section 5.5: "Nesterov outer optimizer improves over averaging"
# =================================================================

import math
import logging

_m085_logger = logging.getLogger("DeepSpeed")


class DeslocWSDSchedule:
    """Warmup-Stable-Decay (WSD) learning rate schedule.

    Implements the 3-phase schedule from DES-LOC Section 4.1:
      Phase 1 (warmup):  linear from 0 → peak_lr over warmup_steps
      Phase 2 (stable):  constant at peak_lr
      Phase 3 (decay):   cosine decay from peak_lr → min_lr

    Used as the inner learning rate for DES-LOC training.
    """

    def __init__(self, optimizer, peak_lr, min_lr=0.0,
                 warmup_steps=512, stable_steps=4000,
                 decay_steps=500, last_step=-1):
        self.optimizer = optimizer
        self.peak_lr = peak_lr
        self.min_lr = min_lr
        self.warmup_steps = max(1, warmup_steps)
        self.stable_steps = max(0, stable_steps)
        self.decay_steps = max(1, decay_steps)
        self.total_steps = (self.warmup_steps + self.stable_steps
                            + self.decay_steps)
        self._step_count = last_step + 1
        self._lr_history = []

    def get_lr_at_step(self, step):
        if step < self.warmup_steps:
            ...
            ratio = min(1.0, decay_progress / self.decay_steps)
            cos_val = 0.5 * (1.0 + math.cos(math.pi * ratio))
    ...
    def get_last_lr(self):
        if self._lr_history:
            return self._lr_history[-1]
        return self.get_lr_at_step(self._step_count)
    ...
    def load_state_dict(self, state):
        ...
        self.peak_lr = state.get("peak_lr", self.peak_lr)
        self.min_lr = state.get("min_lr", self.min_lr)

    def get_lr_curve(self, num_steps=None):
        n = num_steps or self.total_steps
        return [(s, self.get_lr_at_step(s)) for s in range(n)]


class NesterovOuterOptimizer:
    """Nesterov momentum as DES-LOC outer optimizer.

    DES-LOC Section 5.5: "using SGDM as the outer optimizer improves
    performance over averaging by ≈ 0.015 nats"

    Instead of simple averaging after AllReduce, this applies
    Nesterov momentum to the outer synchronization step:
      v_{t+1} = β_outer * v_t + (x_avg - x_local)
      x_local = x_avg + β_outer * v_{t+1}

    Theory (Eq.4): higher momentum sync frequency (pu) permits
    a larger step size: η₀ ∝ 1/√ψ
    """
    ...
    def __init__(self, momentum=0.9, outer_lr=1.0,
        ...
        self._velocity_buffers = {}
        self._step = 0
        self._applied_count = 0

    def apply_outer_step(self, param_id, local_param, averaged_param):
        """Apply Nesterov outer step after AllReduce averaging.

        Args:
            param_id: unique identifier for this parameter
            local_param: the local worker's parameter tensor
            averaged_param: the globally-averaged parameter tensor
        """
        try:
            import torch
        except ImportError:
            local_param.copy_(averaged_param)
            return

        # Compute the "outer gradient": difference from average
        outer_grad = averaged_param - local_param

        # Initialize velocity buffer if needed
        if param_id not in self._velocity_buffers:
            self._velocity_buffers[param_id] = torch.zeros_like(
                local_param)

        velocity = self._velocity_buffers[param_id]

        # Update velocity with momentum
        velocity.mul_(self.momentum).add_(
            outer_grad, alpha=1.0 - self.dampening)

        # Apply Nesterov lookahead
        if self.nesterov:
            update = outer_grad + self.momentum * velocity
        else:
            update = velocity

        # Apply outer update
        local_param.add_(update, alpha=self.outer_lr)
        self._applied_count += 1

    def apply_simple_averaging(self, local_param, averaged_param):
        """Fallback: simple averaging without momentum."""
        local_param.copy_(averaged_param)

    def reset(self):
        self._velocity_buffers.clear()
        self._step = 0
    ...
    def get_stats(self):
        return {
            ...
            "step": self._step,
            "num_params": len(self._velocity_buffers),
    ...
    def state_dict(self):
        return {
            ...
            "outer_lr": self.outer_lr,
            "step": self._step,
            # Velocity buffers serialized separately
        }


class CheckpointInitializer:
    """Initialize DES-LOC from DDP checkpoint (warm-start).

    DES-LOC Section 5.3: "DES-LOC effectively initializes new
    synchronization states and outperforms ad-hoc averaging"

    T_WARM = 512 steps of DDP training before switching to DES-LOC.
    """

    def __init__(self, warmup_steps=512, ddp_checkpoint_path=None):
        self.warmup_steps = warmup_steps
        self.ddp_checkpoint_path = ddp_checkpoint_path
        self._initialized = False
        self._init_step = 0

    def load_ddp_checkpoint(self, model, optimizer, path=None):
        """Load a DDP-trained checkpoint into model/optimizer."""
        path = path or self.ddp_checkpoint_path
        if path is None:
            _m085_logger.info("No DDP checkpoint — training from scratch")
            return False
        try:
            import torch
            ...
            if "optimizer_state_dict" in ckpt and optimizer is not None:
                try:
                    ...
                    _m085_logger.info(
                        "Optimizer state mismatch — reinitializing")
            self._init_step = ckpt.get("step", 0)
            self._initialized = True
            _m085_logger.info(
                f"Loaded DDP checkpoint from step {self._init_step}")
            return True
        except Exception as e:
            _m085_logger.warning(f"Checkpoint load failed: {e}")
            return False

    def is_in_warmup(self, current_step):
        return current_step < self._init_step + self.warmup_steps

    def should_use_ddp(self, current_step):
        """During warmup, use DDP (sync every step)."""
        return self.is_in_warmup(current_step)

    def should_switch_to_desloc(self, current_step):
        """Return True at the exact transition point."""
        boundary = self._init_step + self.warmup_steps
        return current_step == boundary
    ...
    def get_info(self):
        return {
            "warmup_steps": self.warmup_steps,
            "ddp_checkpoint": self.ddp_checkpoint_path,
            "initialized": self._initialized,
            "init_step": self._init_step,
            "transition_step": self._init_step + self.warmup_steps,
        }


class DeslocPsiCalculator:
    """Compute the ψ factor from DES-LOC Theorem 1.

    ψ = 4(1-px)/px² · (1-β)(1-pu) / (6(1-(1-pu)β))

    This controls the higher-order convergence term and the
    relationship between sync frequency and step size.
    Smaller ψ → can use larger step size.
    """

    @staticmethod
    def compute_psi(px, pu, beta):
        """Compute ψ factor.

        px = 1/Kx (parameter sync probability)
        pu = 1/Ku (momentum sync probability)
        β = momentum coefficient (β₁)
        """
        if px <= 0 or px > 1:
            return float("inf")
        if pu <= 0 or pu > 1:
        ...
        numerator = 4.0 * (1.0 - px) * (1.0 - beta) * (1.0 - pu)
        denominator = px * px * 6.0 * (1.0 - (1.0 - pu) * beta)
        if abs(denominator) < 1e-30:
            return float("inf")
        return numerator / denominator

    @staticmethod
    def max_step_size(psi, L=1.0):
        """Maximum step size η₀ ∝ 1/√ψ from Theorem 1."""
        if psi <= 0 or math.isinf(psi):
            return 0.0
        return 1.0 / (L * math.sqrt(psi))

    @staticmethod
    def analyze_sync_config(Kx, Ku, beta1=0.9):
        """Analyze a DES-LOC sync configuration."""
        px = 1.0 / Kx
        pu = 1.0 / Ku
        psi = DeslocPsiCalculator.compute_psi(px, pu, beta1)
        max_eta = DeslocPsiCalculator.max_step_size(psi)
        return {
            "Kx": Kx, "Ku": Ku,
            "px": px, "pu": pu,
            "beta1": beta1,
            "psi": round(psi, 4),
            "max_step_size": round(max_eta, 6),
        }


# =================================================================
# End M085  (WSD + Nesterov Outer + Checkpoint Init + Psi Calculator)
# =================================================================
