...
# =================================================================
# M078: DES-LOC Benchmark Config Matrix — 14 benchmarks, hardware-aware
# Claude-5 (M077-M091)
# =================================================================

DESLOC_BENCH_SUITE_KEY = "desloc_benchmark_suite"
DESLOC_BENCH_HARDWARE_KEY = "desloc_hardware_profile"

# GPU hardware specifications for auto-adaptation
_GPU_PROFILES = {
    "A6000": {
        "memory_gb": 48, "peak_tflops_bf16": 155.0,
        "pcie_bw_gbps": 64, "nvlink": False,
        "max_batch_117M": 64, "max_batch_350M": 32,
        "max_batch_1B": 8,
    },
    "H100_NVL": {
        "memory_gb": 94, "peak_tflops_bf16": 1979.0,
        "pcie_bw_gbps": 64, "nvlink": True,
        "nvlink_bw_gbps": 900,
        "max_batch_117M": 128, "max_batch_350M": 64,
        "max_batch_1B": 32,
    },
    "H100_SXM": {
        "memory_gb": 80, "peak_tflops_bf16": 1979.0,
        "pcie_bw_gbps": 64, "nvlink": True,
        "nvlink_bw_gbps": 900,
        "max_batch_117M": 128, "max_batch_350M": 64,
        "max_batch_1B": 24,
...
class HardwareProfile:
    """Auto-detect or manually specify GPU configuration."""

    def __init__(self, gpu_names=None, auto_detect=True):
        self.gpus = []
        if gpu_names:
            for name in gpu_names:
                profile = _GPU_PROFILES.get(name, _GPU_PROFILES["A6000"])
                self.gpus.append({"name": name, **profile})
        elif auto_detect:
            self.gpus = self._detect_gpus()
        if not self.gpus:
            self.gpus = [{"name": "A6000", **_GPU_PROFILES["A6000"]}]

    @staticmethod
    def _detect_gpus():
        """Try to detect GPUs via torch.cuda."""
        detected = []
        try:
            import torch
            if torch.cuda.is_available():
                for i in range(torch.cuda.device_count()):
                    ...
                    profile = _GPU_PROFILES.get("A6000").copy()
                    mem = torch.cuda.get_device_properties(i).total_mem
                    profile["memory_gb"] = mem / (1024 ** 3)
                    if "H100" in name:
                        profile.update(_GPU_PROFILES["H100_NVL"])
                    elif "A6000" in name:
                        profile.update(_GPU_PROFILES["A6000"])
                    profile["name"] = name
    ...
    def min_memory_gb(self):
        if not self.gpus:
            return 0
        return min(g["memory_gb"] for g in self.gpus)

    def total_memory_gb(self):
        return sum(g["memory_gb"] for g in self.gpus)

    def max_batch_for_model(self, model_size_str):
        """Return safe batch size for the weakest GPU."""
        key = f"max_batch_{model_size_str}"
        vals = [g.get(key, 8) for g in self.gpus]
        return min(vals) if vals else 8

    def is_heterogeneous(self):
        names = set(g["name"] for g in self.gpus)
        return len(names) > 1

    def to_dict(self):
        return {"gpus": self.gpus, "num_gpus": self.num_gpus(),
                "heterogeneous": self.is_heterogeneous()}


class BenchmarkDefinition:
    """One benchmark specification with all its parameters."""

    def __init__(self, bench_id, method, description, **kwargs):
        self.bench_id = bench_id
        self.method = method
        self.description = description
        self.model_size = kwargs.get("model_size", "117M")
        self.Kx = kwargs.get("Kx", 1)
        self.Ku = kwargs.get("Ku", 1)
        self.Kv = kwargs.get("Kv", 1)
        self.beta1 = kwargs.get("beta1", 0.9)
        self.beta2 = kwargs.get("beta2", 0.999)
        self.inner_optimizer = kwargs.get("inner_optimizer", "adam")
        self.outer_optimizer = kwargs.get("outer_optimizer", "averaging")
        self.total_steps = kwargs.get("total_steps", 5000)
        self.warmup_steps = kwargs.get("warmup_steps", 512)
        self.learning_rate = kwargs.get("learning_rate", 3e-4)
        self.batch_size = kwargs.get("batch_size", None)
        self.seq_len = kwargs.get("seq_len", 1024)
        self.seeds = kwargs.get("seeds", [42, 137, 2024])
        self.section = kwargs.get("section", "5.3")
...
# The 14 NeurIPS benchmarks
DESLOC_BENCHMARK_DEFINITIONS = [
    BenchmarkDefinition(
        "bench01_ddp_baseline_117M", "ddp",
        "DDP baseline GPT-2 117M (Section 5.3)",
        model_size="117M", section="5.3"),
    BenchmarkDefinition(
        "bench02_localadam_Kx8_117M", "local_adam",
        "Local Adam Kx=8 GPT-2 117M baseline (Section 5.3)",
        model_size="117M", Kx=8, section="5.3"),
    BenchmarkDefinition(
        "bench03_desloc_Kx8_Ku24_Kv48", "desloc",
        "DES-LOC Kx=8,Ku=24,Kv=48 comm reduction (Section 5.3)",
        model_size="117M", Kx=8, Ku=24, Kv=48, section="5.3"),
    BenchmarkDefinition(
        "bench04_desloc_Kx16_Ku48_Kv96", "desloc",
        "DES-LOC Kx=16,Ku=48,Kv=96 aggressive comm (Section 5.3)",
        model_size="117M", Kx=16, Ku=48, Kv=96, section="5.3"),
    BenchmarkDefinition(
        "bench05_Kx_sweep", "desloc",
        "Kx parameter sweep: 2,4,8,16,32,64 (Section 5.2)",
        model_size="117M", section="5.2"),
    BenchmarkDefinition(
        "bench06_Ku_sweep", "desloc",
        "Ku momentum sync sweep (Section 5.2)",
        model_size="117M", Kx=8, section="5.2"),
    BenchmarkDefinition(
        "bench07_beta2_halflife", "desloc",
        "β₂ half-life verification (Section 5.1)",
        model_size="117M", Kx=8, section="5.1"),
    BenchmarkDefinition(
        "bench08_adopt_variant", "desloc",
        "ADOPT optimizer variant (Section 5.4)",
        model_size="117M", inner_optimizer="adopt",
        Kx=8, Ku=24, Kv=48, section="5.4"),
    BenchmarkDefinition(
        "bench09_nesterov_outer", "desloc_outer",
        "Nesterov outer optimizer (Section 5.5)",
        model_size="117M", outer_optimizer="nesterov",
        Kx=8, section="5.5"),
    BenchmarkDefinition(
        "bench10_muon_inner", "desloc",
        "Muon inner optimizer integration (Section 5.6)",
        model_size="117M", inner_optimizer="muon",
        Kx=8, Ku=24, section="5.6"),
    BenchmarkDefinition(
        "bench11_350M_scaling", "desloc",
        "GPT-2 350M scale validation (Section 5.4)",
        model_size="350M", Kx=8, Ku=24, Kv=48, section="5.4"),
    BenchmarkDefinition(
        "bench12_comm_roofline", "desloc",
        "Communication roofline analysis (Appendix)",
        model_size="117M", Kx=8, section="appendix"),
    BenchmarkDefinition(
        "bench13_convergence_rate", "desloc",
        "Convergence rate ‖∇f‖² vs T (Section 3)",
        model_size="117M", Kx=8, Ku=24, Kv=48,
        total_steps=10000, section="3"),
    BenchmarkDefinition(
        "bench14_checkpoint_init", "desloc",
        "Checkpoint warm-start validation (Section 5.3)",
        model_size="117M", Kx=8, Ku=24, Kv=48,
        warmup_steps=512, section="5.3"),
]


class DeslocBenchmarkSuite:
    """Manage the full set of 14 benchmarks with hardware adaptation."""

    def __init__(self, benchmarks=None, hardware=None):
        self.benchmarks = benchmarks or DESLOC_BENCHMARK_DEFINITIONS
        self.hardware = hardware or HardwareProfile()
        self._adapt_batch_sizes()

    def _adapt_batch_sizes(self):
        """Auto-set batch sizes based on GPU memory."""
        for bench in self.benchmarks:
            if bench.batch_size is None:
                bench.batch_size = self.hardware.max_batch_for_model(
                    bench.model_size)

    def get_benchmark(self, bench_id):
        for b in self.benchmarks:
            if b.bench_id == bench_id:
                return b
        return None

    def list_benchmarks(self):
        return [(b.bench_id, b.description) for b in self.benchmarks]

    def count(self):
        return len(self.benchmarks)

    def total_runs_with_seeds(self):
        return sum(len(b.seeds) for b in self.benchmarks)

    def get_by_section(self, section):
        return [b for b in self.benchmarks if b.section == section]

    def validate(self):
        """Check all configs are internally consistent."""
        errors = []
        for b in self.benchmarks:
            if b.Ku < b.Kx:
                errors.append(f"{b.bench_id}: Ku({b.Ku}) < Kx({b.Kx})")
            if b.Kv < b.Ku:
                errors.append(f"{b.bench_id}: Kv({b.Kv}) < Ku({b.Ku})")
            if b.warmup_steps >= b.total_steps:
                errors.append(
                    f"{b.bench_id}: warmup({b.warmup_steps}) "
                    f">= total({b.total_steps})")
            if b.beta1 >= b.beta2 and b.inner_optimizer == "adam":
                errors.append(
                    f"{b.bench_id}: β₁({b.beta1}) >= β₂({b.beta2})")
        return errors

    def to_dict(self):
        return {
            "num_benchmarks": self.count(),
            "total_runs": self.total_runs_with_seeds(),
            "hardware": self.hardware.to_dict(),
            "benchmarks": [b.to_dict() for b in self.benchmarks],
        }


def parse_desloc_benchmark_suite(param_dict):
    """Entry point for parsing benchmark suite from DS config JSON."""
    hw_cfg = param_dict.get(DESLOC_BENCH_HARDWARE_KEY, {})
    hw = HardwareProfile(
        gpu_names=hw_cfg.get("gpu_names"),
        auto_detect=hw_cfg.get("auto_detect", True),
    )
    suite = DeslocBenchmarkSuite(hardware=hw)
    errors = suite.validate()
    if errors:
        import logging
        logger = logging.getLogger("DeepSpeed")
        for e in errors:
            logger.warning(f"DES-LOC benchmark validation: {e}")
    return suite


# =================================================================
# End M078  (Benchmark Suite + Hardware Profile + 14 Benchmarks)
# =================================================================
