...
def get_desloc_compile_state():
    return dict(_desloc_init_cfg)


def desloc_should_skip_allreduce(step=None):
    if not _desloc_init_cfg['enabled']:
        return False
    s = step if step is not None else _desloc_init_cfg['step']
    Kx = _desloc_init_cfg['Kx']
    if Kx <= 1:
        return False
    return s % Kx != 0
