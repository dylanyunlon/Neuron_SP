...
#   2. initialize_model_parallel() signature: trailing underscore params removed,
#      typed Optional[int] for virtual/split args.
#   3. Remove rank-0 print inside initialize_model_parallel (moved to caller in
#      megatron/initialize.py, now also mirrored in core.initialize_model_parallel).
#   4. Replace ensure_divisibility() call with explicit RuntimeError raise.
#   5. Type-annotate local variables (world_size, data_parallel_size, num_*).
#   6. Remove blank lines and trailing backslash wraps for style.
#   7. destroy_model_parallel(): add cleanup of _VIRTUAL_PIPELINE_MODEL_PARALLEL_*,
#      _MPU_TENSOR/PIPELINE_MODEL_PARALLEL_WORLD_SIZE/RANK globals.
#
...
def is_pipeline_stage_at_split():
    """Return true if pipeline stage executes decoder block and is the
    first stage after the split."""
    rank = get_pipeline_model_parallel_rank()
    return is_pipeline_stage_after_split(rank) and \
        not is_pipeline_stage_after_split(rank - 1)
...
def get_pipeline_model_parallel_first_rank():
    """Return the global rank of the first stage in pipeline for the
    current rank."""
    assert _PIPELINE_GLOBAL_RANKS is not None, \
        "Pipeline parallel group is not initialized"
    return _PIPELINE_GLOBAL_RANKS[0]
...
def get_pipeline_model_parallel_last_rank():
    """Return the global rank of the last stage in pipeline for the
    current rank."""
    assert _PIPELINE_GLOBAL_RANKS is not None, \
        "Pipeline parallel group is not initialized"
