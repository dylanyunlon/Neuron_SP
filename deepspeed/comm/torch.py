...
# =================================================================
# M081: Trainium2 Compat Layer — NVIDIA↔AWS Neuron Comm Abstraction
# Claude-5 (M077-M091)
# Nick Joseph: "它们实现的方式相当不同,编程方式也很不同"
# =================================================================
...
_logger = logging.getLogger("DeepSpeed")


class NeuronCommOpsMapper:
    """Map NCCL collective ops to Neuron CC ops equivalents.

    Anthropic trains on AWS Trainium2 via EFA (Elastic Fabric Adapter).
    This mapper provides the translation layer so DES-LOC code is
    hardware-agnostic.
    """

    # Op mapping: NCCL name → Neuron equivalent
    _OP_MAP = {
    ...
    # Data type mapping: torch dtype → Neuron dtype
    _DTYPE_MAP = {
        "torch.float32": "f32",
        "torch.float16": "f16",
        "torch.bfloat16": "bf16",
    ...
    @classmethod
    def get_neuron_op(cls, nccl_op_name):
        return cls._OP_MAP.get(nccl_op_name, nccl_op_name)

    @classmethod
    def get_neuron_dtype(cls, torch_dtype_str):
        return cls._DTYPE_MAP.get(str(torch_dtype_str), "f32")

    @classmethod
    def is_neuron_available(cls):
        """Check if AWS Neuron runtime is available."""
        try:
            import torch_xla
            return True
        except ImportError:
        ...
        try:
            ...
            pass
        return os.environ.get("NEURON_RT_VISIBLE_CORES") is not None

    @classmethod
    def get_backend_name(cls):
        if cls.is_neuron_available():
            return "neuron_xla"
        return "nccl"


class NeuronCollectiveWrapper:
    """Wrap collective operations for Neuron/NCCL transparency.

    Falls back to NCCL when Neuron is not available.
    Tracks performance counters for both backends.
    """

    def __init__(self, backend=None):
        self._backend = backend or NeuronCommOpsMapper.get_backend_name()
        self._call_count = 0
        self._total_bytes = 0
        self._total_time_ms = 0.0
        self._op_counts = {}
        self._is_neuron = self._backend == "neuron_xla"

    def all_reduce(self, tensor, op="sum", group=None):
        """AllReduce with backend dispatch."""
        self._call_count += 1
        self._op_counts["all_reduce"] = (
            self._op_counts.get("all_reduce", 0) + 1)
        self._total_bytes += tensor.numel() * tensor.element_size()

        if self._is_neuron:
            return self._neuron_all_reduce(tensor, op, group)
        else:
            return self._nccl_all_reduce(tensor, op, group)

    def _nccl_all_reduce(self, tensor, op, group):
        """Standard NCCL AllReduce."""
        try:
            import deepspeed.comm as dist
            return dist.all_reduce(tensor, group=group)
        except Exception as e:
            _logger.warning(f"NCCL all_reduce failed: {e}")
            return None

    def _neuron_all_reduce(self, tensor, op, group):
        """Neuron XLA AllReduce — falls back to NCCL if unavailable."""
        try:
            import torch_xla.core.xla_model as xm
            return xm.all_reduce(op, [tensor], groups=group)
        except ImportError:
            _logger.debug("Neuron XLA not available, falling back to NCCL")
            return self._nccl_all_reduce(tensor, op, group)

    def reduce_scatter(self, output, input_tensor, op="sum",
                       group=None):
        self._call_count += 1
        self._op_counts["reduce_scatter"] = (
            self._op_counts.get("reduce_scatter", 0) + 1)
        self._total_bytes += input_tensor.numel() * input_tensor.element_size()

        if self._is_neuron:
            return self._neuron_reduce_scatter(
                output, input_tensor, op, group)
        else:
            return self._nccl_reduce_scatter(
                output, input_tensor, op, group)

    def _nccl_reduce_scatter(self, output, input_tensor, op, group):
        try:
            import deepspeed.comm as dist
            return dist.reduce_scatter(output, input_tensor, group=group)
        except Exception as e:
            _logger.warning(f"NCCL reduce_scatter failed: {e}")
            return None

    def _neuron_reduce_scatter(self, output, input_tensor, op, group):
        try:
            import torch_xla.core.xla_model as xm
            return xm.reduce_scatter(op, input_tensor,
                                     scale=1.0, groups=group)
        except ImportError:
            return self._nccl_reduce_scatter(
                output, input_tensor, op, group)

    def all_gather(self, output_list, tensor, group=None):
        self._call_count += 1
        self._op_counts["all_gather"] = (
            self._op_counts.get("all_gather", 0) + 1)
        self._total_bytes += tensor.numel() * tensor.element_size()

        if self._is_neuron:
            try:
                import torch_xla.core.xla_model as xm
                return xm.all_gather(tensor, groups=group)
            except ImportError:
                pass
        try:
            import deepspeed.comm as dist
            return dist.all_gather(output_list, tensor, group=group)
        except Exception as e:
            _logger.warning(f"all_gather failed: {e}")
            return None

    def broadcast(self, tensor, src=0, group=None):
        self._call_count += 1
        self._op_counts["broadcast"] = (
            self._op_counts.get("broadcast", 0) + 1)
        if self._is_neuron:
            try:
                import torch_xla.core.xla_model as xm
                xm.collective_broadcast([tensor], src)
                return
            except (ImportError, AttributeError):
                pass
        try:
            import deepspeed.comm as dist
            return dist.broadcast(tensor, src=src, group=group)
        except Exception as e:
            _logger.warning(f"broadcast failed: {e}")

    def get_stats(self):
        return {
            "backend": self._backend,
            "is_neuron": self._is_neuron,
            "total_calls": self._call_count,
            "total_bytes": self._total_bytes,
            "total_bytes_gb": round(self._total_bytes / 1e9, 3),
...
class EFABandwidthEstimator:
    """Estimate AWS EFA (Elastic Fabric Adapter) bandwidth.

    EFA v2: 400 Gbps per instance (Trn2)
    NVLink v4: 900 GB/s (H100 SXM)
    PCIe Gen5: 64 GB/s

    Provides cost/performance comparison for Trainium2 vs NVIDIA.
    """

    INTERCONNECT_SPECS = {
        "efa_v1": {"bandwidth_gbps": 100, "latency_us": 12},
        "efa_v2": {"bandwidth_gbps": 400, "latency_us": 8},
        "nvlink_v3": {"bandwidth_gbps": 600, "latency_us": 1},
        "nvlink_v4": {"bandwidth_gbps": 900, "latency_us": 0.5},
        "pcie_gen4": {"bandwidth_gbps": 32, "latency_us": 5},
        "pcie_gen5": {"bandwidth_gbps": 64, "latency_us": 3},
        "infiniband_hdr": {"bandwidth_gbps": 200, "latency_us": 1},
        "infiniband_ndr": {"bandwidth_gbps": 400, "latency_us": 0.6},
    }

    @classmethod
    def estimate_allreduce_time(cls, msg_size_bytes, world_size,
                                interconnect="nvlink_v4"):
        """Estimate AllReduce time using ring algorithm model.

        T = latency * 2*(N-1) + msg_size * 2*(N-1)/(N*BW)
        """
        spec = cls.INTERCONNECT_SPECS.get(
            interconnect, cls.INTERCONNECT_SPECS["pcie_gen4"])
        bw_bytes = spec["bandwidth_gbps"] * 1e9 / 8  # Gbps → B/s
        lat = spec["latency_us"] * 1e-6  # μs → s
        n = world_size
        if n <= 1:
            return 0.0
        ring_factor = 2.0 * (n - 1) / n
        time_s = (lat * 2 * (n - 1)
                  + msg_size_bytes * ring_factor / bw_bytes)
        return time_s * 1000  # ms

    @classmethod
    def compare_interconnects(cls, msg_size_bytes, world_size,
                              interconnects=None):
        """Compare AllReduce time across interconnect types."""
        if interconnects is None:
            interconnects = ["efa_v2", "nvlink_v4", "pcie_gen5"]
        results = {}
        for ic in interconnects:
            t = cls.estimate_allreduce_time(
                msg_size_bytes, world_size, ic)
            results[ic] = {"time_ms": round(t, 4),
                           "spec": cls.INTERCONNECT_SPECS.get(ic, {})}
        return results

    @classmethod
    def desloc_comm_savings(cls, param_bytes, Kx, Ku, Kv,
                            total_steps, world_size,
                            interconnect="nvlink_v4"):
        """Estimate wall-clock savings from DES-LOC vs DDP."""
        t_one = cls.estimate_allreduce_time(
            param_bytes, world_size, interconnect)

        # DDP: 1 AllReduce per step (gradients)
        ddp_time = total_steps * t_one

        # Local Adam: 3 AllReduces every Kx steps
        local_adam_syncs = (total_steps // Kx) * 3
        local_adam_time = local_adam_syncs * t_one

        # DES-LOC: param every Kx, u every Ku, v every Kv
        desloc_syncs = ((total_steps // Kx)
                        + (total_steps // Ku)
                        + (total_steps // Kv))
        desloc_time = desloc_syncs * t_one

        return {
            "ddp_comm_time_ms": round(ddp_time, 2),
            "local_adam_comm_time_ms": round(local_adam_time, 2),
            "desloc_comm_time_ms": round(desloc_time, 2),
            "savings_vs_ddp_ms": round(ddp_time - desloc_time, 2),
            "savings_vs_local_adam_ms": round(
                local_adam_time - desloc_time, 2),
            "speedup_vs_ddp": round(
                ddp_time / max(0.001, desloc_time), 1),
            "speedup_vs_local_adam": round(
                local_adam_time / max(0.001, desloc_time), 1),
        }


# =================================================================
# End M081  (Neuron Comm Layer + EFA Bandwidth Estimator)
# =================================================================
