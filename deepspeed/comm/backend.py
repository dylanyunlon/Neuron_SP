...
# =================================================================
# M082: Hardware Diagnostics — GPU Health Check & Precision Validator
# Claude-5 (M077-M091)
# Nick Joseph: "以前我写代码出错会想'肯定是我写错了',
# 但现在有时真的是'电脑错了'"
# =================================================================
...
_m082_logger = logging.getLogger("DeepSpeed")


class PrecisionValidator:
    """Validate GPU computation precision against CPU reference.

    Nick Joseph: "如果有初创公司能检测每块芯片的精确性并报告故障,非常有用"

    Runs small matmul tests on GPU, compares against CPU float64 result.
    Detects silent data corruption that does not raise exceptions.
    """

    def __init__(self, tolerance_fp32=1e-5, tolerance_fp16=1e-2,
                 tolerance_bf16=1e-2, matrix_size=256):
        self.tolerances = {
            "float32": tolerance_fp32,
            "float16": tolerance_fp16,
            "bfloat16": tolerance_bf16,
        }
        self.matrix_size = matrix_size
        self._results = []

    def validate_device(self, device_id=0):
        """Run precision check on a single GPU."""
        try:
            import torch
        except ImportError:
            return {"device": device_id, "status": "skip",
        ...
        results = {"device": device_id, "device_name": "",
                   "checks": [], "status": "pass"}

        try:
            results["device_name"] = torch.cuda.get_device_name(device_id)
        except Exception:
            results["device_name"] = "unknown"

        n = self.matrix_size
        # Generate deterministic test matrices using torch
        torch.manual_seed(42 + device_id)
        a_cpu = torch.randn(n, n, dtype=torch.float64)
        b_cpu = torch.randn(n, n, dtype=torch.float64)
        ref = torch.matmul(a_cpu, b_cpu)

        for dtype_name, tol in self.tolerances.items():
            dtype = getattr(torch, dtype_name, None)
            if dtype is None:
                continue
            try:
                a_gpu = a_cpu.to(dtype).to(device)
                b_gpu = b_cpu.to(dtype).to(device)
                c_gpu = torch.matmul(a_gpu, b_gpu)
                c_cpu = c_gpu.to(torch.float64).cpu()
                max_err = (c_cpu - ref).abs().max().item()
                rel_err = max_err / (ref.abs().max().item() + 1e-30)
                passed = rel_err < tol
                check = {
                    "dtype": dtype_name,
                ...
                if not passed:
                    results["status"] = "FAIL"
                    _m082_logger.warning(
                        f"GPU {device_id} precision FAIL: "
                        f"{dtype_name} rel_err={rel_err:.6e} > tol={tol}")
                results["checks"].append(check)
                del a_gpu, b_gpu, c_gpu, c_cpu
            except Exception as e:
                results["checks"].append({
                    "dtype": dtype_name, "error": str(e),
                    "passed": False,
                })
    ...
    def validate_all_devices(self):
        """Run precision check on all available GPUs."""
        try:
            import torch
...
class ThermalMonitor:
    """Monitor GPU temperature and power during training.

    Records snapshots and detects thermal throttling.
    """

    def __init__(self, warning_temp_c=80, critical_temp_c=90,
                 power_limit_pct=0.95):
        self.warning_temp_c = warning_temp_c
        self.critical_temp_c = critical_temp_c
        self.power_limit_pct = power_limit_pct
        self._snapshots = []
        self._alerts = []

    def snapshot(self):
        """Take a thermal/power snapshot of all GPUs."""
        try:
            import torch
            if not torch.cuda.is_available():
                return {}
        ...
        snap = {"timestamp": time.time(), "gpus": []}
        try:
            # Use pynvml if available for accurate readings
            import pynvml
            pynvml.nvmlInit()
            ...
            for i in range(count):
                ...
                gpu_info = {
                    ...
                    "power_pct": power / max(1, power_limit),
                    "memory_used_mb": mem_info.used / (1024 * 1024),
                    "memory_total_mb": mem_info.total / (1024 * 1024),
                }
                snap["gpus"].append(gpu_info)

                # Check thermal alerts
                if temp >= self.critical_temp_c:
                    alert = (f"CRITICAL GPU {i}: {temp}°C >= "
                             f"{self.critical_temp_c}°C")
                    self._alerts.append(alert)
                elif temp >= self.warning_temp_c:
                    alert = (f"WARNING GPU {i}: {temp}°C >= "
                             f"{self.warning_temp_c}°C")
                    self._alerts.append(alert)
            pynvml.nvmlShutdown()
        except ImportError:
            # Fallback: use torch.cuda for memory only
            import torch
            ...
            _m082_logger.debug(f"ThermalMonitor error: {e}")
...
class HardwareDiagnostics:
    """Complete hardware diagnostics suite for DES-LOC training.

    Runs before training to catch faulty GPUs early.
    """

    def __init__(self, precision_validator=None, thermal_monitor=None):
        self.precision = precision_validator or PrecisionValidator()
        self.thermal = thermal_monitor or ThermalMonitor()

    def run_full_check(self):
        """Execute all diagnostics. Returns overall status dict."""
        report = {
        ...
        prec_results = self.precision.validate_all_devices()
        report["precision"] = {
            "results": prec_results,
            "all_passed": self.precision.all_passed(),
        }
        if not self.precision.all_passed():
            report["status"] = "FAIL_PRECISION"

        # Thermal check
        thermal_snap = self.thermal.snapshot()
        report["thermal"] = {
            "snapshot": thermal_snap,
            "alerts": self.thermal.get_alerts(),
            "max_temp": self.thermal.max_temperature(),
        }
        if self.thermal.get_alerts():
            if report["status"] == "pass":
                report["status"] = "WARNING_THERMAL"
...
# =================================================================
# End M082  (PrecisionValidator + ThermalMonitor + Diagnostics)
# =================================================================
