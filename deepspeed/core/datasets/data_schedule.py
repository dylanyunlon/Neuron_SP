...
from deepspeed.core.pipeline_parallel.hybrid_cp_schedule import BalancedCPScheduler
...
class HybridCPDataLoaderWrapper:
    ...
    def __next__(self) -> Any:
        ...
        DES-LOC: monitors subsample distribution across heterogeneous GPU ranks.
        """
        ...
        print(f"[DES-LOC] HybridCP __next__: {len(subsample_seqlens)} subsamples, "
              f"seqlen range=[{subsample_seqlens.min().item() if len(subsample_seqlens) > 0 else 0}, "
              f"{subsample_seqlens.max().item() if len(subsample_seqlens) > 0 else 0}]")
