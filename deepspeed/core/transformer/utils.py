...
# DeepSpeed Team
#
# Ported from Megatron-LM megatron/core/transformer/utils.py
...
from deepspeed.core.dist_checkpointing.mapping import ShardedStateDict, StateDict
...
# ShardedObject: prefer the Megatron version if available, otherwise use a
# lightweight stand-in that is API-compatible with the parts used below.
try:
...
def toggle_cuda_graphs(model, set_to="none"):
    ...
    for attribute, modules in cuda_graph_attr_cache[model_id].items():
        if attribute == "cuda_graph_impl":
            ...
            for module in modules:
                if set_to == "local":
                    # If we are turning on cuda graphs we need to turn off activation recomputation
