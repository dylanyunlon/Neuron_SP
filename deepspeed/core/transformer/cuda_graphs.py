...
from deepspeed.core.num_microbatches_calculator import get_num_microbatches
...
from deepspeed.core.transformer.enums import CudaGraphModule
from deepspeed.core.transformer.module import MegatronModule
try:
    from deepspeed.core.transformer.module import GraphableMegatronModule
except ImportError:
    GraphableMegatronModule = MegatronModule
...
def _check_supported_type(meta):
    ...
    from deepspeed.core.inference.contexts.dynamic_context import DynamicInferenceContext
    from deepspeed.core.inference.contexts.static_context import StaticInferenceContext
...
def _determine_if_first_last_layer_of_this_vp_chunk(base_module):
    ...
    from deepspeed.core.transformer.transformer_block import get_num_layers_to_build
...
def create_cudagraphs():
    ...
    `deepspeed.core.pipeline_parallel.schedules.py`. During the first step, _CudaGraphRunners
...
class _CudaGraphRunner(torch.nn.Module):
    ...
    def __init__(
        ...
        if hasattr(self.base_module, "config") and isinstance(
            ...
            if self.fp4_enabled:
                from deepspeed.core.fp4_utils import get_fp4_recipe  # to avoid circular import
    ...
    def get_quantization_context(self):
        ...
        if self.fp8_runtime_enabled:
            from deepspeed.core.fp8_utils import get_fp8_context  # to avoid circular import

            return get_fp8_context(self.base_module.config, self.base_module.layer_number - 1)
        elif self.fp4_runtime_enabled:
            from deepspeed.core.fp4_utils import get_fp4_context  # to avoid circular import
    ...
    def create_fwd_graph(self, args, kwargs, outputs=None, clone_inputs=True):
        ...
        from deepspeed.core.transformer.transformer_layer import MoETransformerLayer

        is_moe = isinstance(self.base_module, MoETransformerLayer)
        if is_moe:
            from deepspeed.core.transformer.moe.moe_logging import get_moe_metrics_tracker
...
class CudaGraphManager(torch.nn.Module):
    ...
    def __init__(
        ...
        try:
            from deepspeed.core.extensions.transformer_engine import TECudaRNGStatesTracker
        except ImportError:
    ...
    def call_ddp_preforward_hook(self, module):
        ...
        from deepspeed.core.distributed import distributed_data_parallel
...
def _layer_is_graphable(layer, config):
    ...
    # import modules here to avoid a circular import
    from deepspeed.core.ssm.mamba_layer import MambaLayer
...
class TECudaGraphHelper:
    ...
    def _get_cuda_graph_input_data(self):
        ...
        def get_make_graphed_callables_kwargs():
            ...
            from deepspeed.core.fp4_utils import get_fp4_recipe
            from deepspeed.core.fp8_utils import get_fp8_recipe
            ...
            if self.config.fp8 or self.config.fp4:
                ...
                def _get_fp8_enabled():
                    if is_te_min_version("2.8.0"):
                        from deepspeed.core.fp8_utils import is_first_last_bf16_layer
    ...
    def _reset_after_capture(self):
        ...
        """
        from deepspeed.core.distributed.finalize_model_grads import reset_model_temporary_tensors
        from deepspeed.core.transformer.moe.moe_logging import get_moe_metrics_tracker
