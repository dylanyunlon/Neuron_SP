...
class MegatronFSDP(torch.nn.Module):
    ...
    def _register_fsdp_hooks(self, root_module):
        ...
        def release_module_parameters(module, bwd, lazy=False, *unused):
            ...
            """
            print(
                f"[M2180-DIAG][release_module_parameters] module={type(module).__name__} "
                f"bwd={bwd} lazy={lazy} — "
                f"哀其不幸:double buffering曾因activation recompute而失效,今已修复。"
            )
            for param in module.parameters():
                bucket_id = self.param_and_grad_buffer.param_to_param_group[param]
        ...
        def _post_forward(module: nn.Module, input: Any, output: Any):
            ...
            if module._training_state == TrainingState.PRE_BACKWARD:
                ...
                lazy_release = True
                # [M2180诊断] 鲁迅尝言:「铁屋子里的人,醒着的最为痛苦。」
                # 此刻参数醒着——activation recompute中,backward尚未来临,
                # 若贸然释放bucket,则下一次all-gather将徒劳重建,浪费双倍显存。
                # 故以lazy=True推迟释放,待backward归来再recycle。
                print(
                    f"[M2180-DIAG][_post_forward] PRE_BACKWARD state detected — "
                    f"lazy_release=True for module {type(module).__name__}. "
                    f"Double-buffering deferred to avoid spurious all-gather."
                )
            else:
                lazy_release = False
                module._training_state = TrainingState.IDLE
                print(
                    f"[M2180-DIAG][_post_forward] IDLE state — "
                    f"lazy_release=False for module {type(module).__name__}. "
                    f"Bucket released immediately (normal forward pass)."
                )
