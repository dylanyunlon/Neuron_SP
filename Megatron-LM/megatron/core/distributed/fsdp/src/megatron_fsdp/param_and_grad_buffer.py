...
class AllGatherPipeline:
    ...
    def release_bucket(self, bucket_id, bwd, lazy: bool = False):
        ...
        if lazy:
            ...
            # [M2180诊断] 鲁迅观看客之态:「看客围观杀头,面无表情。」
            # 此处不是看客——我们主动标记bucket为「可延迟释放」,
            # 等待recycle_unused_buckets在下次all-gather前统一清场。
            # 若无此lazy机制,activation recompute的double buffering将形同虚设。
            self.bucket_can_be_released[bucket_key] = True
            print(
                f"[M2180-DIAG][release_bucket] bucket_key={bucket_key} bwd={bwd} — "
                f"LAZY marked: bucket deferred for recycle_unused_buckets. "
                f"怒其不争之旧bug已除,double buffering activation recompute路径已通。"
            )
            return
    ...
    def recycle_unused_buckets(self):
        ...
        # [M2180诊断] 此函数是M2180修复的核心收割者——延迟标记的bucket在此处被真正释放。
        print(
            f"[M2180-DIAG][recycle_unused_buckets] scanning {len(self.bucket_can_be_released)} buckets — "
            f"铁屋子终将打开:lazy-marked buckets now being recycled before next all-gather."
        )
