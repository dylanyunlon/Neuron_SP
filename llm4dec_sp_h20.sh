...
ONDEVICE_MODELS=("125M" "350M" "700M" "1.3B" "1.7B" "7B")
...
setup_environment() {
    ...
    # Try conda; if missing, use system python directly (H20 / RunPod bare metal)
    if command -v conda &>/dev/null; then
        activate_env
    else
        log_info "No conda — using system python: $PYTHON_BIN"
    fi

    # Install torch if missing (auto-detect CUDA version)
    if ! "$PYTHON_BIN" -c "import torch" 2>/dev/null; then
        log_info "torch not found — installing..."
        local cuda_ver
        cuda_ver=$(nvidia-smi 2>/dev/null | grep -oP 'CUDA Version: \K[0-9]+\.[0-9]+' || echo "12.4")
        local cu_tag="cu$(echo "$cuda_ver" | tr -d '.'| cut -c1-3)"
        # Try matching CUDA wheel first, fall back to default
        "$PYTHON_BIN" -m pip install torch --index-url "https://download.pytorch.org/whl/${cu_tag}" 2>/dev/null \
            || "$PYTHON_BIN" -m pip install torch 2>/dev/null \
            || { log_error "Failed to install torch"; exit 1; }
    fi

    log_info "Installing remaining packages..."
    "$PYTHON_BIN" -m pip install -q matplotlib seaborn pandas scipy pyyaml tqdm 2>/dev/null || true

    # Patch REAL_GPU_BENCHMARK.py: add 7B/3B if missing
    if [ -f "$PROJECT_DIR/REAL_GPU_BENCHMARK.py" ]; then
        if ! grep -q '"7B"' "$PROJECT_DIR/REAL_GPU_BENCHMARK.py"; then
            log_info "Patching REAL_GPU_BENCHMARK.py for 7B/3B support..."
            sed -i '/"1.7B":.*n_embd.*2304/a\            "3B": {"n_layer": 32, "n_head": 32, "n_embd": 3200},\n            "7B": {"n_layer": 32, "n_head": 32, "n_embd": 4096},' "$PROJECT_DIR/REAL_GPU_BENCHMARK.py"
            sed -i "s/choices=\['125M', '350M', '700M', '1.3B', '1.7B'\]/choices=['125M', '350M', '700M', '1.3B', '1.7B', '3B', '7B']/" "$PROJECT_DIR/REAL_GPU_BENCHMARK.py"
        fi
    fi
    ...
    # Re-detect python now that torch is installed
    PYTHON_BIN="$(_find_python)"
    log_info "Python: $PYTHON_BIN"
    "$PYTHON_BIN" -c "import torch; print(f'torch {torch.__version__}, CUDA {torch.cuda.is_available()}, GPUs {torch.cuda.device_count()}')"
...
_run_single_experiment() {
    ...
    # Memory-aware batch sizing for large models (H20 96GB / A6000 49GB)
    local BS=$BATCH_SIZE GA=$GRAD_ACCUM EXTRA_ARGS=""
    case "$MODEL" in
        7B)      BS=1; GA=16; EXTRA_ARGS="--use_ac" ;;
        3B)      BS=1; GA=8;  EXTRA_ARGS="--use_ac" ;;
        1.3B|1.7B) BS=2; GA=8; EXTRA_ARGS="--use_ac" ;;
    esac
...
run_training() {
    ...
    # Auto-setup: install torch + patch REAL_GPU_BENCHMARK.py if needed
    if ! "$PYTHON_BIN" -c "import torch" 2>/dev/null; then
        setup_environment
    fi
    # Ensure NCCL vars are set even if setup was skipped
    export TORCH_NCCL_ASYNC_ERROR_HANDLING=1
    export NCCL_TIMEOUT=1800000
    export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC=1800
    export NCCL_DEBUG=WARN
...
_show_eval_summary() {
    ...
    local comm_red="1.0"
    [ "$desloc_syncs" -gt 0 ] && comm_red=$(awk -v a="$ddp_syncs" -v b="$desloc_syncs" 'BEGIN{printf "%.1f",a/b}')

    # LocalAdam: syncs all 3 every Kx
    local local_syncs=$(( (steps / kx) * 3 ))
    [ $local_syncs -eq 0 ] && local_syncs=3
    local local_red=$(awk -v a="$ddp_syncs" -v b="$local_syncs" 'BEGIN{printf "%.1f",a/b}')
    ...
    if [ -n "$ddp_loss" ] && [ -n "$desloc_loss" ]; then
        loss_delta_dl=$(awk -v a="$desloc_loss" -v b="$ddp_loss" 'BEGIN{printf "%+.4f",a-b}')
    else
        loss_delta_dl="N/A (run train first)"
    fi
    if [ -n "$ddp_loss" ] && [ -n "$local_loss" ]; then
        loss_delta_la=$(awk -v a="$local_loss" -v b="$ddp_loss" 'BEGIN{printf "%+.4f",a-b}')
    else
        loss_delta_la="N/A"
    fi
...
compare_results() {
...

    {
        ...
        printf "| LocalAdam | %d | %d | %d | %d | %.1f× |\n" \
            ...
            "$(awk -v a="$ddp_syncs" -v b="$la_total" 'BEGIN{printf "%.1f",a/b}')"
        printf "| DES-LOC | %d | %d | %d | %d | %.1f× |\n" \
            "$dl_x" "$dl_u" "$dl_v" "$dl_total" \
            "$(awk -v a="$ddp_syncs" -v b="$dl_total" 'BEGIN{printf "%.1f",a/b}')"
        echo ""
